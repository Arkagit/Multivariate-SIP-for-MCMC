...
pdf("plot_frob.pdf", height = 5, width = 5)
par(mar = c(5.1, 4.8, 4.1, 2.1))
...
plot(samp_size, colMeans(norm_bmopt), ylim = c(min(colMeans(norm_bmth), colMeans(norm_bmopt)), max(colMeans(norm_bmth), colMeans(norm_bmopt))), 
	type = 'l', col = "blue", ylab = "Frobenius Norm", xlab =  "Chain Length")
...
#abline(h = 0, lty = 2)
legend("bottomright", bty = "n",legend = c("BM(optimal)", "BM(Theory)"), 
	col = c("blue", "skyblue"), lty = 2, cex=0.8)
...
pdf("plot_ess.pdf", height = 5, width = 5)
par(mar = c(5.1, 4.8, 4.1, 2.1))
